...
export function useCalendar(orgId) {
  ...
  const handleAddClientToGroupSlots = useCallback(async (groupId, clientId, fromDate) => {
    await addClientToGroupSlots(orgId, groupId, clientId, fromDate)
    fetchSlots()
  }, [orgId, fetchSlots])

  const handleRemoveClientFromGroupSlots = useCallback(async (groupId, clientId, fromDate) => {
    await removeClientFromGroupSlots(orgId, groupId, clientId, fromDate)
    fetchSlots()
  }, [orgId, fetchSlots])

  // ── Controllo max sessioni mensili ────────────────────────────────────
