...
const MOCK_CLIENTS = [
  {
    id: 1, name: "Marco Rossi", level: 12, rank: "Warrior",
    avatar: "💪", xp: 3400, xpNext: 4000,
    stats: { forza: 78, resistenza: 65, flessibilita: 45, velocita: 58, recupero: 72 },
    log: [
      { date: "07 Mar", action: "Completato: Chest Day", xp: +120 },
      { date: "05 Mar", action: "PR Panca: 100kg 🏆", xp: +250 },
      { date: "03 Mar", action: "Streak 7 giorni 🔥", xp: +180 },
    ],
    badges: ["🏆 First PR", "🔥 7-Day Streak", "💎 Consistency King"],
  },
  {
    id: 2, name: "Sofia Bianchi", level: 7, rank: "Scout",
    avatar: "🧘", xp: 1800, xpNext: 2200,
    stats: { forza: 42, resistenza: 80, flessibilita: 88, velocita: 67, recupero: 55 },
    log: [
      { date: "08 Mar", action: "Yoga Flow 60min", xp: +90 },
      { date: "06 Mar", action: "5km Run: 28min", xp: +150 },
    ],
    badges: ["🧘 Zen Master", "🏃 Road Runner"],
  },
  {
    id: 3, name: "Luca Ferrari", level: 3, rank: "Rookie",
    avatar: "🌱", xp: 450, xpNext: 700,
    stats: { forza: 30, resistenza: 35, flessibilita: 28, velocita: 40, recupero: 38 },
    log: [
      { date: "08 Mar", action: "Prima sessione completata!", xp: +50 },
    ],
...
function Pentagon({ stats, color }) {
  ...
  const dataPoints = labels.map((k, i) => getPoint(i, stats[k]));
  const dataPath = dataPoints.map((p, i) => `${i === 0 ? "M" : "L"}${p.x},${p.y}`).join(" ") + "Z";

  return (
    <svg width={size} height={size} style={{ overflow: "visible" }}>
      {[0.25, 0.5, 0.75, 1].map((r) => (
        <polygon key={r} points={gridPoints(r)}
      ...
      {labels.map((k, i) => {
        ...
        return (
          <text key={k} x={lx} y={ly} textAnchor="middle" dominantBaseline="middle"
            fontSize="8" fill="rgba(255,255,255,0.6)" fontFamily="'Rajdhani', sans-serif">
            {STAT_ICONS[k]}
          </text>
        );
      })}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
      ...
      <div style={{ textAlign: "center" }}>
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.4)", fontFamily: "Rajdhani, sans-serif", letterSpacing: 2, textTransform: "uppercase" }}>Rank</div>
        <div style={{ color, fontFamily: "'Orbitron', monospace", fontSize: 16, fontWeight: 700 }}>{client.rank}</div>
      </div>
    </div>
  );
}
...
function Modal({ title, onClose, children }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center" }}
      onClick={onClose}>
      <div style={{ background: "#111827", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 20, padding: 32, width: 400, maxWidth: "90vw" }}
  ...
  return (
    <Modal title="➕ Nuovo Cliente" onClose={onClose}>
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        <input style={inputStyle} placeholder="Nome e Cognome" value={form.name}
        ...
        <button onClick={() => { if (form.name) { onAdd(form); onClose(); } }}
          style={{ background: "linear-gradient(135deg, #3b82f6, #8b5cf6)", border: "none", borderRadius: 12, padding: "14px", color: "#fff", fontFamily: "Orbitron, monospace", fontSize: 13, fontWeight: 700, cursor: "pointer", letterSpacing: 1 }}>
          AGGIUNGI CLIENTE
        </button>
      </div>
    </Modal>
...
function LevelUpModal({ client, onClose, onLevelUp }) {
  ...
  return (
    <Modal title={`⬆️ Level Up — ${client.name}`} onClose={onClose}>
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <div style={{ color: "rgba(255,255,255,0.5)", fontSize: 13, fontFamily: "Rajdhani, sans-serif" }}>
          Aggiorna i parametri per avanzare al prossimo livello
        </div>
        {STAT_LABELS.map(k => (
          <div key={k} style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <span style={{ width: 100, color: "rgba(255,255,255,0.6)", fontSize: 13, fontFamily: "Rajdhani, sans-serif", textTransform: "capitalize" }}>
              {STAT_ICONS[k]} {k}
            </span>
            <span style={{ width: 28, color: "#fff", fontSize: 13, fontFamily: "Orbitron, monospace" }}>{client.stats[k]}</span>
            <input type="range" min={-10} max={20} value={deltas[k]}
              onChange={e => setDeltas({ ...deltas, [k]: +e.target.value })}
        ...
        <textarea style={{ ...inputStyle, resize: "vertical", minHeight: 60 }}
          placeholder="Note sessione..." value={note} onChange={e => setNote(e.target.value)} />
        <button onClick={() => { onLevelUp(deltas, note); onClose(); }}
          style={{ background: "linear-gradient(135deg, #f59e0b, #ef4444)", border: "none", borderRadius: 12, padding: "14px", color: "#fff", fontFamily: "Orbitron, monospace", fontSize: 13, fontWeight: 700, cursor: "pointer", letterSpacing: 1 }}>
          🚀 CONFERMA LEVEL UP
        </button>
      </div>
    </Modal>
...
// ─── TRAINER AREA ────────────────────────────────────────────────────────────
function TrainerArea({ onSelectClient }) {
  ...
  const addClient = ({ name, avatar }) => {
    setClients([...clients, {
      id: Date.now(), name, avatar, level: 1, rank: "Rookie", xp: 0, xpNext: 700,
      stats: { forza: 20, resistenza: 20, flessibilita: 20, velocita: 20, recupero: 20 },
      log: [{ date: new Date().toLocaleDateString("it-IT", { day: "2-digit", month: "short" }), action: "Benvenuto nel programma! 🎉", xp: +50 }],
      badges: ["🌱 New Challenger"],
    }]);
  };
  ...
  return (
    <div style={{ padding: "40px 32px", maxWidth: 600, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
        <input style={{ ...inputStyle, flex: 1 }} placeholder="🔍 Cerca cliente..."
          value={search} onChange={e => setSearch(e.target.value)} />
        <button onClick={() => setShowAdd(true)}
          style={{ background: "linear-gradient(135deg, #3b82f6, #8b5cf6)", border: "none", borderRadius: 12, padding: "10px 20px", color: "#fff", fontFamily: "Orbitron, monospace", fontSize: 12, cursor: "pointer", whiteSpace: "nowrap" }}>
          + AGGIUNGI
        </button>
      </div>
...
function ClientDashboard({ client: initialClient, onBack }) {
  ...
  const handleLevelUp = (deltas, note) => {
    const newStats = {};
    ...
    const newLog = [{ date: today, action: note || `Sessione di aggiornamento (Lv.${newLevel})`, xp: +xpGain }, ...client.log];
    const RANKS = ["Rookie", "Scout", "Warrior", "Champion", "Legend"];
    const newRank = RANKS[Math.min(Math.floor(newLevel / 4), RANKS.length - 1)];
    setClient({ ...client, stats: newStats, xp: newXpFinal, xpNext: newXpNext, level: newLevel, rank: newRank, log: newLog });
  };
  ...
  return (
    <div style={{ padding: "28px 20px", maxWidth: 700, margin: "0 auto" }}>
      ...
      <Card style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "32px 20px", marginBottom: 16 }}>
        <Avatar client={client} color={color} />
        ...
        <Card>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {client.badges.map((b, i) => (
              <div key={i} style={{ background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: "8px 12px", fontFamily: "Rajdhani, sans-serif", fontSize: 14, color: "rgba(255,255,255,0.8)", display: "flex", alignItems: "center", gap: 8 }}>
                {b}
              </div>
            ))}
          </div>
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @keyframes pulse {
          0%, 100% { box-shadow: 0 0 20px var(--c, #60a5fa55); }
          50% { box-shadow: 0 0 50px var(--c, #60a5fa99); }
        }
      ...
      <div style={{ minHeight: "100vh", background: "radial-gradient(ellipse at 20% 0%, #0f1f3d 0%, #070b14 60%)", color: "#fff" }}>
        ...
        <div style={{ padding: "16px 24px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ flex: 1 }} />
          <span style={{ fontFamily: "Rajdhani, sans-serif", fontSize: 13, color: "rgba(255,255,255,0.3)" }}>
        ...
        {selectedClient
          ...
          : <TrainerArea onSelectClient={setSelectedClient} />
        }
      </div>
    </>
  );
